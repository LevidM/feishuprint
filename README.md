...
## 飞书开发平台配置与本地调试
如果你本地打开 `http://localhost:5173/` 一直显示“初始化中”或“不在飞书环境”，这是正常现象：插件依赖飞书容器注入的 Bitable SDK，脱离飞书页面无法拿到表格上下文。

按以下步骤配置：
1. 进入飞书开放平台（开发者后台），创建“多维表格插件（侧边栏）”。
2. 在插件配置里设置“上线地址”或“开发地址”为你的可访问 HTTPS 地址。
   - 本地调试可使用内网穿透（如 `https://xxxx.ngrok-free.app`）转发到 `http://localhost:5173`。
3. 在权限配置中勾选需要的多维表格读取权限（至少包含读取表、字段、记录相关权限）。
4. 发布开发版本（或保存配置）后，回到目标多维表格。
5. 在右侧插件面板中添加并打开该插件。
6. 这时插件才能正确初始化并读取当前表/视图数据。

排查建议：
- 如果仍初始化失败，先确认当前 URL 是在飞书多维表格内打开，而不是浏览器直接访问本地地址。
- 检查插件配置的地址是否与本地端口一致，且 HTTPS 证书可访问。
- 检查插件权限是否完整，尤其是字段/记录读取权限。
